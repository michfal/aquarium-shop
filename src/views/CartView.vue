        ...
        <div class="grid lg:grid-cols-[minmax(0,2fr)_minmax(260px,1fr)] gap-8">
          ...
          <section>
            ...
            <div class="bg-white rounded-2xl shadow-md overflow-hidden">
              <!-- Header row -->
              <div
                class="hidden md:grid grid-cols-[minmax(0,2.5fr)_repeat(3,minmax(0,1fr))] gap-4 px-6 py-4 text-xs font-semibold uppercase text-slate-500 border-b border-slate-100 tracking-wide"
              >
                <span>Product</span>
              ...
              <div
                v-if="isEmpty"
                class="px-4 md:px-6 py-4 flex flex-col gap-4 border-b text-slate-500 border-slate-100 text-xs font-semibold uppercase"
              >
              ...
              <div
                ...
                v-for="p in products"
                :key="p.id"
                class="px-4 md:px-6 py-4 flex flex-col gap-4 border-b border-slate-100 md:grid md:grid-cols-[minmax(0,2.5fr)_repeat(3,minmax(0,1fr))] md:items-center"
              >
                <!-- Produkt -->
                ...
                <div class="md:text-center">
                  <div
                    class="inline-flex items-center border border-slate-300 rounded-lg overflow-hidden bg-white"
                  >
                    <button
                      @click="dec(p.id)"
                      class="px-2 py-1 text-sm text-slate-500 hover:bg-slate-50"
                    >
                      -
                    </button>
                    <input
          ...
          <aside class="space-y-6">
            <section class="bg-white rounded-2xl shadow-md p-5">
              ...
              <div class="mt-3">
                <button
                  class="px-8 py-2.5 rounded-full bg-sky-600 text-white text-sm font-semibold shadow w-full hover:bg-sky-700 transition"
                >
                  CHECKOUT
                </button>
              </div>
